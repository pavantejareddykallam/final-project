...
let namep;
app.post("/loginpatient", auth, async (req, res) => {
...
app.post("/checkpatient", async (req, res) => {
    try {
        ...
        if (await Register2.findOne({ email: email })) {
            vis = user.vists;
            res.status(201).render(`patientoptions`, { adminname: adminnamelogo, username: usernamelogo, visitCount: vis });
        }
        else {
...
app.get("/checkpatient", (req, res) => {
    res.render("patientoptions", { adminname: adminnamelogo, username: "userName", visitCount: "visitC" });
});
